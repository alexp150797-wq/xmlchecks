...
def _style(ws):
    ...
    ws.conditional_formatting.add(f"O2:O{ws.max_row}", CellIsRule(operator='equal', formula=['"OK"'], fill=GREEN))
    ws.conditional_formatting.add(f"O2:O{ws.max_row}", CellIsRule(operator='notEqual', formula=['"OK"'], fill=RED))
